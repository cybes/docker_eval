...
ENV ZOO_CONF_DIR=/conf \
    ZOO_DATA_DIR=/data \
    ZOO_DATA_LOG_DIR=/zookeeper-3.4.14/logs \
    ZOO_LOG_DIR=/ \
    ZOO_TICK_TIME=2000 \
    ZOO_INIT_LIMIT=5 \
    ZOO_SYNC_LIMIT=2 \
...
RUN set -eux; \
    ...
    mkdir -p "$ZOO_DATA_LOG_DIR" "$ZOO_DATA_DIR" "$ZOO_CONF_DIR" "$ZOO_LOG_DIR" "/data/zookeeper_data"; \
...
RUN set -eux; \
    ...
    mv  /$DISTRO_NAME/bin/zkEnv.sh /$DISTRO_NAME/bin/zkEnv-sh-orig; \
...
COPY log4j.properties zoo.cfg $DISTRO_NAME/conf/
COPY zkEnv.sh /$DISTRO_NAME/bin/
